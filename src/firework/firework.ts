...
class Firework {
    ...
    private strength: number =
        Math.random() * (FireworkConfig.maxStrength - FireworkConfig.minStrength) + FireworkConfig.minStrength;
    private color: string =
        `${Math.trunc(Math.random() * 255)},${Math.trunc(Math.random() * 255)},` + `${Math.trunc(Math.random() * 255)}`;
